...
def loadStreams(streams_file, COBALT_ID):
  while(True):
      ...
      if not line.startswith("#"):
        ...
        while (not opened):
          if(len(cores_current) < cores_max):
            t = threading.Thread(target=download, args=(line, COBALT_ID,))
            t.start()
...
if __name__ == '__main__':
  ...
  loadStreams(sys.argv[1], sys.argv[2])
